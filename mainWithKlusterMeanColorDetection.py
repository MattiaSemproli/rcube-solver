...
def run():
    ...
    def closest_color(color, colors):
        ...
        closest_col = None
        for col in colors:
            dist = distance.euclidean(color, col['rgb'])
            if dist < min_dist:
                min_dist = dist
                closest_col = col['name']
        return closest_col
    ...
    def draw_roi_cells(roi):
        ...
        for k, v in cells.items():
            ...
            # Estrazione della cella
            cell = roi[y:y+h, x:x+w]
            
            # Calcolo del colore dominante
            dominant_color = get_dominant_color(cell)
            
            # Convertire il colore in formato RGB per visualizzazione
            dominant_color_rgb = [int(c) for c in dominant_color]
            
            # Disegna il colore dominante sulla cella
            # cv.rectangle(roi, (x, y), (x + w, y + h), dominant_color_rgb, -1)

            print(f"{k}: {closest_color(dominant_color_rgb, colors)}")
            if k == "X1":
                print("\n")
